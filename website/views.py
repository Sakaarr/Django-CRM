...
def home(request):
    ...
    if request.method=='POST':
        ...
        if user is not None:
            ...
            messages.success(request , 'There was an error login you in')
            return redirect('home')
    else:
...
def delete_record(request,pk):
    if request.user.is_authenticated:
        ...
        messages.success(request, "You need to be loggied in to use this feature")
        return redirect('home')
...
def update_record(request , pk):
    if request.user.is_authenticated:
        ...
        if form.is_valid():
            form.save()
            messages.success(request , 'Records have been Updated')
            return redirect('home')
